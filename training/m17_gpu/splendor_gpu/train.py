...
def train_one(spec: ModelSpec, train_set: SplendorDataset, validation_set: SplendorDataset, config: dict[str, Any], out_dir: Path, catalog_source: Path) -> dict[str, Any]:
    ...
    report = {"format": "effective-splendor-gpu-training-report", "version": 1, "training_id": config["training_id"], "model_id": metadata["model_id"], "device": str(device), "torch_version": torch.__version__, "cuda_version": torch.version.cuda, "gpu_name": torch.cuda.get_device_name(device) if device.type == "cuda" else None, "elapsed_seconds": time.perf_counter() - start, "parameter_count": sum(p.numel() for p in model.parameters()), "metric_semantics": {"validation_kind": "offline_source_held_out_teacher_fit", "diagnostic_only": True, "strength_authority": "arena_rating_or_promotion_gate"}, "selection": {"metric": "policy_nll + value_loss_weight * value_mse", "best_epoch": best_epoch, "best_score": best_score}, "baselines": {"uniform_policy_nll": metrics["uniform_policy_nll"], "train_prior_value": train_value_prior.tolist(), "train_prior_value_mse": value_prior_mse}, "checkpoint_hash": checkpoint_hash, "checkpoint_file_sha256": checkpoint_file_sha256, "validation": metrics, "history": history}
...
def main() -> None:
    ...
    print(json.dumps({"out_dir": str(args.out_dir), "models": [{"model_id": r["model_id"], "offline_validation": r["validation"]} for r in reports]}))
