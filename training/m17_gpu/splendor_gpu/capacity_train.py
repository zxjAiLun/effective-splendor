"""M28A fresh-init Entity Mixer width-scaling trainer.

This module owns the M28A training contract instead of extending the historic
fine-tuning entry points.  The two M28A models are always built from a frozen
``ModelSpec`` and never loaded from an inherited checkpoint.
"""
...
CONFIG_FORMAT = "effective-splendor-m28a-entity-mixer-width-scaling"
TRAINING_CONFIG_HASH_DOMAIN = b"effective-splendor-m28a-capacity-training-config-v1\0"
EXPECTED_BASELINE_COMMIT = "428c227f507a232be0aab9187e3195f8c352f4bd"
EXPECTED_CATALOG_HASH = "4c90cb85d565e74af3e955df62d431174aaf5a8d4192895f95c8d21d57d78a26"

EXPECTED_MODELS: tuple[dict[str, Any], ...] = (
    {
        "role": "control",
        "model_id": "m28a-entity-mixer-h192-b4-control-v1",
    ...
    {
        "role": "candidate",
        "model_id": "m28a-entity-mixer-h320-b4-v1",
        "architecture": "entity_mixer",
        "hidden_dim": 320,
        "blocks": 4,
        "dropout": 0.0,
        "expected_parameter_count": 2605764,
    },
)
...
def training_config_hash(config: dict[str, Any]) -> str:
    """Return the semantic identity of the frozen preregistration config."""

    return hashlib.sha256(TRAINING_CONFIG_HASH_DOMAIN + json.dumps(
...
def validate_config(config: dict[str, Any]) -> None:
    """Fail closed unless the tracked M28A prereg is exactly the frozen one."""

    _assert_equal(config.get("format"), CONFIG_FORMAT, "config format")
    _assert_equal(config.get("version"), 1, "config version")
    _assert_equal(config.get("milestone"), "M28A", "milestone")
    ...
    _assert_equal(config.get("training_authorization"), "NOT_AUTHORIZED", "top-level training authorization")
    _assert_equal(config.get("arena_authorization"), "NOT_AUTHORIZED", "top-level Arena authorization")
    _assert_equal(config.get("promotion"), "NONE", "promotion")
    _assert_equal(config.get("champion"), "M07", "champion")

    parent = config.get("parent")
    if not isinstance(parent, dict):
        raise ValueError("M27A parent binding is missing")
    _assert_equal(parent.get("milestone"), "M27A", "parent milestone")
    _assert_equal(parent.get("status"), "ACCEPTED / CLOSED", "parent status")
    _assert_equal(parent.get("closure_commit"), EXPECTED_BASELINE_COMMIT, "M27A closure commit")
    _assert_equal(parent.get("outcome"), "M27A_INCONCLUSIVE", "parent outcome")

    authorization = config.get("authorization")
    if not isinstance(authorization, dict):
        raise ValueError("authorization contract is missing")
    _assert_equal(authorization.get("training"), "NOT_AUTHORIZED", "training authorization")
    _assert_equal(authorization.get("arena"), "NOT_AUTHORIZED", "Arena authorization")
    ...
    _assert_equal(initialization.get("initialization_seed"), 280129, "initialization seed")
    _assert_equal(initialization.get("shuffle_seed"), 280129, "shuffle seed")
    forbidden = initialization.get("forbidden_sources")
    if forbidden != [
        ...
        raise ValueError("fresh initialization forbidden-source contract changed")
    ...
    _assert_equal(
        offline.get("relative_improvement_formula"),
        "floor(10000 * (control - candidate) / control)",
        "relative improvement formula",
    )
    _assert_equal(offline.get("top1_delta_formula"), "candidate_top1 - control_top1", "top1 delta formula")
    ...
    _assert_equal(g1.get("policy_ce_improvement_min_bps"), 50, "G1 policy threshold")
    _assert_equal(g1.get("value_mse_improvement_min_bps"), 50, "G1 value threshold")
    _assert_equal(g1.get("policy_ce_non_regression_min_bps"), -100, "G1 policy non-regression")
    _assert_equal(g1.get("value_mse_non_regression_min_bps"), -100, "G1 value non-regression")
    _assert_equal(g1.get("top1_delta_min"), -0.010, "G1 top1 non-regression")
    _assert_equal(g2.get("policy_ce_improvement_min_bps"), -100, "G2 policy threshold")
    _assert_equal(g2.get("value_mse_improvement_min_bps"), -100, "G2 value threshold")
    _assert_equal(g2.get("top1_delta_min"), -0.010, "G2 top1 non-regression")
    _assert_equal(offline.get("fail_decision"), "M28A_OFFLINE_NO_CAPACITY_SIGNAL", "offline fail decision")
    ...
    m07 = arena.get("m07")
    timeouts = arena.get("timeouts_ms")
    matrix = arena.get("matrix")
    statistics = arena.get("statistics")
    if not all(isinstance(value, dict) for value in (neural, m07, timeouts, matrix, statistics)):
        raise ValueError("Arena screen contract is incomplete")
    _assert_equal(neural.get("simulations"), 16, "Arena neural simulations")
    _assert_equal(neural.get("max_depth_turns"), 1, "Arena neural depth")
    _assert_equal(neural.get("puct_exploration_milli"), 1500, "Arena PUCT")
    _assert_equal(neural.get("sample_seed"), 28000018, "Arena neural sample seed")
    _assert_equal(neural.get("device"), "cuda", "Arena neural device")
    _assert_equal(m07.get("sample_seed"), 20260810, "M07 sample seed")
    _assert_equal(m07.get("sample_count"), 4, "M07 sample count")
    _assert_equal(m07.get("max_depth_turns"), 1, "M07 depth")
    _assert_equal(m07.get("max_nodes"), 2000, "M07 max nodes")
    _assert_equal(timeouts, {"handshake": 5000, "move": 30000, "shutdown": 2000}, "Arena timeouts")
    _assert_equal(
        matrix.get("pairs"),
        ["candidate_vs_control", "candidate_vs_m07", "control_vs_m07"],
        "Arena pairs",
    )
    _assert_equal(matrix.get("game_seeds"), list(range(302001, 302033)), "Arena seeds")
    ...
    _assert_equal(statistics.get("direct_capacity_threshold_bps"), 5500, "direct capacity threshold")
    _assert_equal(statistics.get("m07_anchor_threshold_bps"), 500, "M07 anchor threshold")
    _assert_equal(statistics.get("uncertainty_role"), "diagnostic_only", "uncertainty role")
...
def validate_dataset(dataset_path: Path, config: dict[str, Any]) -> tuple[dict[str, Any], str, str]:
    """Validate both the semantic self-play identity and the raw file identity."""
...
def split_m28a_examples(payload: dict[str, Any], config: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    """Return train, full S2 validation, and the frozen S1-reference subset."""
    ...
    if any(int(example["game_index"]) % 4 == 0 for example in train):
        raise ValueError("validation games leaked into the training partition")
...
def build_fresh_model(model_contract: dict[str, Any], seed: int) -> nn.Module:
    """Build one M28A model from scratch after resetting the frozen seed."""
    ...
    _assert_equal(
        actual_count,
        int(model_contract["expected_parameter_count"]),
        f"{model_contract['model_id']} parameter count",
    )
    return model
...
def offline_comparison(
    control_report: dict[str, Any], candidate_report: dict[str, Any], config: dict[str, Any]
) -> dict[str, Any]:
    """Apply the preregistered offline gates without consulting Arena results."""
    ...
    for label in ("validation", "s1_reference"):
        ...
        comparisons[label] = {
            "policy_ce_improvement_bps": relative_improvement_bps(
                float(control["policy_cross_entropy"]), float(candidate["policy_cross_entropy"])
            ),
            "value_mse_improvement_bps": relative_improvement_bps(
                float(control["value_mse"]), float(candidate["value_mse"])
            ),
    ...
    g1_pass = (
        (
            full["policy_ce_improvement_bps"] >= int(g1["policy_ce_improvement_min_bps"])
            or full["value_mse_improvement_bps"] >= int(g1["value_mse_improvement_min_bps"])
        )
    ...
    return {
        ...
        "decision": "M28A_OFFLINE_NO_CAPACITY_SIGNAL" if not (g1_pass and g2_pass) else "ARENA_ELIGIBLE_PENDING_AUTHORIZATION",
...
def build_checkpoint_metadata(
    ...
    """Build the v1 checkpoint metadata without consulting an inherited model."""
    ...
    return {
        ...
        **model.checkpoint_metadata(),
        ...
        "training_stage": "m28a_entity_mixer_width_scaling_v1",
...
def train_one(
    ...
    """Train one fresh model and select only from full S2 validation."""

    training = config["training"]
    if os.environ.get("CUBLAS_WORKSPACE_CONFIG") != training["determinism"]["cublas_workspace_config"]:
        raise RuntimeError("CUBLAS_WORKSPACE_CONFIG does not match the frozen M28A recipe")
    device = resolve_device(str(training["device"]))
    seed = int(training["initialization_seed"])
    shuffle_seed = int(training["shuffle_seed"])
    model = build_fresh_model(model_contract, seed).to(device)
    ...
    train_loader = _loader(train_set, int(training["batch_size"]), True, shuffle_seed, device)
    ...
    if best_state is None:
        raise RuntimeError("M28A training produced no checkpoint candidate")
    ...
    report = {
        "format": "effective-splendor-m28a-capacity-training-report",
        "version": 1,
        "milestone": "M28A",
        "model_id": model_contract["model_id"],
        "model_role": role,
        ...
        "metric_semantics": {
            ...
            "strength_authority": "frozen_arena_screen_only",
        },
        "selection": {
            "metric": "policy_cross_entropy + 0.5 * value_mse",
            "best_epoch": best_epoch,
            "best_score": best_score,
            "source": "full S2 validation only",
        },
        "validation": validation_metrics,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train the frozen M28A width-scaling pair.")
    parser.add_argument("--dataset", type=Path, required=True)
    parser.add_argument("--config", type=Path, required=True)
    parser.add_argument(
        "--catalog",
        type=Path,
        default=Path("apps/replay-studio/tests/fixtures/rust-analysis-trace-v1.json"),
    )
    ...
    train_examples, validation_examples, reference_examples = split_m28a_examples(payload, config)
    ...
    summary = {
        "format": "effective-splendor-m28a-capacity-training-summary",
        "version": 1,
        "milestone": "M28A",
        "training_config_hash": training_config_hash(config),
