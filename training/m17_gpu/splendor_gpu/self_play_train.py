...
SELF_PLAY_DOMAIN_V1 = b"effective-splendor-neural-self-play-v1\0"
SELF_PLAY_DOMAIN_V2 = b"effective-splendor-neural-self-play-v2\0"


def self_play_hash(payload: dict[str, Any]) -> str:
    version = int(payload.get("version", 0))
    domain = SELF_PLAY_DOMAIN_V2 if version == 2 else SELF_PLAY_DOMAIN_V1
    return hashlib.sha256(
        domain + json.dumps(payload, ensure_ascii=False, separators=(",", ":")).encode("utf-8")
    ).hexdigest()


def dataset_contract(payload: dict[str, Any]) -> tuple[int, str, str]:
    version = int(payload.get("version", 0))
    if version == 1 and payload.get("format") == "effective-splendor-neural-self-play":
        return version, "effective-splendor-neural-self-play-v1\0", "checkpoint_hash"
    if version == 2 and payload.get("format") == "effective-splendor-neural-self-play-v2":
        return version, "effective-splendor-neural-self-play-v2\0", "base_checkpoint_hash"
...
def normalized_visits(example: dict[str, Any]) -> torch.Tensor:
    ...
    if "policy_target_visits" in example:
        expected_visits = [stats[key] for key in legal_keys]
        if list(example["policy_target_visits"]) != expected_visits:
            raise ValueError("policy_target_visits must match action_stats in legal-action order")
        visits = torch.tensor(expected_visits, dtype=torch.float32)
    else:
...
class SelfPlayDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        expected_value_target = torch.tensor(
        ...
        if value_target is not None:
            ...
            if value_target_tensor.shape != (2,):
                raise ValueError("value_target must be viewer-relative length 2")
            if not torch.equal(value_target_tensor, expected_value_target):
                raise ValueError("value_target must match final_ranks")
        else:
            value_target_tensor = expected_value_target
        return {
...
def main() -> None:
    ...
    dataset_version, _dataset_domain, checkpoint_field = dataset_contract(payload)
    ...
    if payload[checkpoint_field] != config["base_checkpoint_hash"]:
        raise ValueError("self-play was not generated by the frozen base checkpoint")
    self_play_config_hash = payload.get("collector_config_hash", payload.get("config_hash"))
    if not self_play_config_hash:
        raise ValueError("self-play config hash is missing")
    ...
    metadata = {
        **base_metadata,
        ...
        "self_play_dataset_version": dataset_version,
        "self_play_config_hash": self_play_config_hash,
