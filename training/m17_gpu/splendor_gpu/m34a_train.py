"""M34A Hierarchical Take-Pattern Policy Training Runner."""
import time
import json
import math
...
if __name__ == "__main__":
    ...
    model_path = Path("training/m17_gpu/splendor_gpu/m34a_model.py")
    encoding_path = Path("training/m17_gpu/splendor_gpu/m34a_encoding.py")
    eval_path = Path("training/m17_gpu/splendor_gpu/m34a_eval.py")
    preflight_path = Path("training/m17_gpu/splendor_gpu/m34a_preflight.py")
    ...
    provenance_hashes = preflight_m34a(
    ...
    print(f"Running M34A Hierarchical Policy on {device} (Init seed={INIT_SEED})...", flush=True)
    ...
    print("Pre-encoding dataset with exact action deltas and hierarchical components...", flush=True)
    t_enc = time.time()
        ...
        def __init__(self, examples, catalog):
            ...
            for ex in examples:
                ...
                for a in ex["legal_actions"]:
                    ...
                    family_indices.append(get_action_family(a))
                    take_pattern_indices.append(get_take_pattern_id(a))
                    return_vectors_6d.append(get_return_vector_6d(a))
                ...
                self.items.append({
                    ...
                    "take_pattern_indices": torch.tensor(take_pattern_indices, dtype=torch.long),
                    "return_vectors_6d": torch.tensor(return_vectors_6d, dtype=torch.float32),
    ...
    def packed_hierarchical_collate(items):
        ...
        take_pat_list = [it["take_pattern_indices"] for it in items]
        return_vec_list = [it["return_vectors_6d"] for it in items]
        ...
        return {
            ...
            "take_pattern_indices": torch.cat(take_pat_list, dim=0),
            "return_vectors_6d": torch.cat(return_vec_list, dim=0),
            "policy_target": torch.cat(policy_list, dim=0),
            "value_target": value_target,
        }

    train_dataset = HierarchicalDataset(train_examples, catalog)
    eval_train_dataset = HierarchicalDataset(train_examples, catalog)
    val_dataset = HierarchicalDataset(val_examples, catalog)
    ...
    train_loader = DataLoader(train_dataset, batch_size=128, shuffle=True, generator=train_generator, collate_fn=packed_hierarchical_collate)
    eval_train_loader = DataLoader(eval_train_dataset, batch_size=128, shuffle=False, collate_fn=packed_hierarchical_collate)
    val_loader = DataLoader(val_dataset, batch_size=128, shuffle=False, collate_fn=packed_hierarchical_collate)
    ...
    for ep in range(1, epochs + 1):
        ...
        val_res = evaluate_m34a_vectorized_fast(model, val_loader, val_H, val_u_ce, device)
    ...
    final_val = evaluate_m34a_diagnostics(model, val_loader, val_examples, val_H, val_u_ce, device)
    final_train = evaluate_m34a_diagnostics(model, eval_train_loader, train_examples, train_H, train_u_ce, device)
    ...
    targeted_signal_pass = (
        ...
        and targeted_take_pat_pass
        and targeted_res_top1_pass
        and targeted_buy_top1_pass
    )

    hierarchical_signal_pass = global_signal_pass and targeted_signal_pass

    if g1_overall_pass:
        decision = "M34A_G1_PASS_AUTHORIZE_G2"
    elif hierarchical_signal_pass:
        decision = "M34A_HIERARCHICAL_SIGNAL_CONFIRMED_G1_FAIL"
    else:
        decision = "STOP_HIERARCHICAL_TAKE_PATTERN_POLICY_ROUTE"

    out_payload = {
        "milestone": "M34A",
        "objective": "HIERARCHICAL_TAKE_PATTERN_POLICY_V1",
        "provenance": {
        ...
        "model": {
            "architecture": "hierarchical_delta_entity_mixer",
        ...
        "comparison_vs_exp_d2_baseline": {
            ...
            "m34a_val_excess_ce": final_val["excess_ce"],
            "m34a_val_top1": final_val["top1"],
            "m34a_val_impr_bps": final_val["impr_bps"],
        ...
        "gate_evaluations": {
            ...
            "hierarchical_signal_gate": {
                ...
                "hierarchical_signal_pass": hierarchical_signal_pass,
            },
            "decision": decision,
    ...
    out_path = Path("benchmarks/m34a-hierarchical-policy.result.json")
    out_path.write_text(json.dumps(out_payload, indent=2) + "\n", encoding="utf-8")
    print(f"COMPLETE M34A: Best Epoch {best_epoch}, Val CE {final_val['ce']:.4f}, Val Top1 {final_val['top1']*100:.2f}%, Take Top1 {final_val['take']['exact_top1']*100:.1f}%, Decision: {decision}", flush=True)
