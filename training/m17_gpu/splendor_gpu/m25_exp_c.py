"""M25 Optimization Recovery - Experiment C: Minimal Interaction Probe (contextual_entity_mixer 1 block, h192/b4, 128 epochs)."""
import time
import json
import math
...
print(f"Running Experiment C (contextual interaction 1 block, h192/b4) on {device}...", flush=True)
...
# Build contextual_entity_mixer h192/b4 with 1 interaction block
spec_c = ModelSpec(
...
model = build_model(spec_c).to(device)
param_count = sum(p.numel() for p in model.parameters())
print(f"Built contextual_entity_mixer (1 interaction block): {param_count:,} parameters (vs control h192/b4 949,060 params)", flush=True)
...
print(f"Starting Experiment C: {epochs} epochs of Interaction Probe Policy-Only (wd=1e-4, lr=3e-4 cosine)...", flush=True)
...
# Load Experiment A baseline for comparison
...
out_payload = {
    "experiment": "M25_RECOVERY_EXP_C_INTERACTION_PROBE",
    "model": {
        "architecture": "contextual_entity_mixer",
        "hidden_dim": 192,
        "blocks": 4,
    ...
    "comparison_vs_control_exp_a": {
        "control_architecture": "entity_mixer",
        ...
        "interaction_val_ce": final_val["ce"],
        "interaction_val_excess_ce": final_val["excess_ce"],
        "interaction_val_top1": final_val["top1"],
...
out_path = Path("benchmarks/m25-recovery-exp-c.result.json")
out_path.write_text(json.dumps(out_payload, indent=2) + "\n", encoding="utf-8")
print(f"COMPLETE: Interaction Best Epoch {best_epoch}, Val CE {final_val['ce']:.4f} (Exc {final_val['excess_ce']:+.4f}), Val Top1 {final_val['top1']*100:.2f}%, Train CE {final_train['ce']:.4f}, Train Top1 {final_train['top1']*100:.2f}%", flush=True)
