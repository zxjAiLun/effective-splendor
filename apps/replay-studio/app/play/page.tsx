...
type Catalog = { cards:DevelopmentCardData[] };
type State = {
...
export default function HumanPlayPage() {
  ...
  useEffect(()=>{ queueMicrotask(()=>void (async()=>{
    try {
      ...
      const [value,catalogValue]=await Promise.all([response.json() as Promise<{agents:Agent[]}>,fetch(`${API}/catalog`).then(result=>{if(!result.ok)throw new Error(`Catalog ${result.status}`);return result.json() as Promise<Catalog>;})]); setAgents(value.agents); setCatalog(catalogValue); setHostOnline(true); setError("");
  ...
  return <main className="human-studio">
    ...
    {state&&<section className="human-workspace">
      <article className="human-board">
        ...
        <div className="human-market">{[2,1,0].map(tier=><div className="human-tier" key={tier}><span>TIER {tier+1}<small>{state.observation.public.deck_counts[tier]} deck</small></span>{state.observation.public.market[tier].map((cardId,slot)=>{const card=cardId==null?null:cards.get(cardId);const action=state.legal_actions.find(item=>item.type==="buy_market"&&item.tier===["One","Two","Three"][tier]&&item.slot===slot);return card?<DevelopmentCard card={card} interactive affordable={Boolean(action)} disabled={busy||!action} onClick={()=>{if(action)void play(action);}} slotLabel={`slot ${slot+1}`} key={slot}/>:<EmptyDevelopmentCard key={slot}/>;})}</div>)}</div>
