"""M45A Arena Orchestrator: Bonus-Vector Information Probe.

Executes the frozen 3 pairings across 64 paired seed blocks x 2 seat rotations = 384 physical matches:
  - 1. SHIFT_F4 vs FULL    (F4 affordability computed with SHIFT1-scrambled bonuses)
  - 2. SHIFT_E2 vs FULL    (E2 noble progress computed with SHIFT1-scrambled bonuses)
  - 3. SHIFT_F4_E2 vs FULL (both color-sensitive paths scrambled)

All agents use the identical n1 static-successor decision shell
(sample_seed=20_260_703, sample_count=4, depth=1, nodes=1).
Fresh seed segment 5_800_000..5_800_063 (no M44 rematches).

Evaluates:
  - Bonferroni-adjusted 98.333% two-sided bootstrap CIs (alpha = 0.05/3)
  - BOOTSTRAP_SEED = 44_300_001, 10,000 resamples
  - Decision rules: RESOLVED_BINDING_IMPORTANT, RESOLVED_BINDING_INTERFERING, UNRESOLVED
    (deliberately NOT the SENSITIVE vocabulary: the treatment is color-identity
    binding, not family deletion)
"""
...
OUT_ROOT = REPO / "local-artifacts/m45a-arena"
...
def run_one_match(
    ...
    game_id = f"m45a-{pairing_id}-b{block_idx:02d}-s{seed}-r{rotation}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="M45A Arena Orchestrator")
    ...
    print("=== M45A Bonus-Vector Information Probe Orchestrator ===")
    ...
    print(f"=======================================================")
    for s in summaries:
        print(f"  {s['pairing_id']:26s}: {s['center_bps']:7.1f} bps  CI: [{s['bootstrap_ci'][0]:6.1f}, {s['bootstrap_ci'][1]:6.1f}]  -> {s['verdict']}")

    out_summary_path = OUT_ROOT / "m45a-arena-summary.json"
