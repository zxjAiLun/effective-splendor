"""M44A Arena Orchestrator: StaticEvaluator Information Attribution.

Executes the frozen 5 pairings across 64 paired seed blocks x 2 seat rotations = 640 physical matches:
...
Evaluates:
  - 4 family drops: Bonferroni-adjusted 98.75% two-sided bootstrap CIs (alpha = 0.05/4)
...
def run_one_match(
    ...
    game_id = f"m44a-{pairing_id}-b{block_idx:02d}-s{seed}-r{rotation}"
...
def run_pairing(spec: dict[str, Any], workers: int = 8) -> dict[str, Any]:
    ...
    # Bonferroni 98.75% for family drops (alpha = 0.05/4), 95% for zero_progress
    if spec["is_family"]:
    ...
    print(
        ...
        f"Score {center_bps:.1f} bps ({ci_level} CI: [{ci_lower:.1f}, {ci_upper:.1f}]), "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="M44A Arena Orchestrator")
    ...
    print("M44A Arena Orchestrator started.", flush=True)
    ...
    summary = {
        "format": "effective-splendor-m44a-orchestrator-summary",
        "version": 1,
    ...
    out_file = OUT_ROOT / "m44a-orchestrator-summary.json"
    out_file.write_text(json.dumps(summary, indent=2), encoding="utf-8")
    print(f"\nAll 5 pairings completed in {total_time:.1f}s. Summary written to {out_file}.", flush=True)
